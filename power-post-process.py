...
def process_generic_redfish_csv(metrics, filename: str, file_id: str):
    ...
    """
    print(f'power-post-process: processing generic-redfish CSV: {filename}')
    ...
    with open(filename, 'r') as file:
        ...
        for row in reader:
            ...
            # Log power consumed
            if row['power_consumed_watts'] != 'N/A':
...
def process_bf3_sensor_csv(metrics, filename: str, file_id: str):
    ...
    """
    print(f'power-post-process: processing bf3-sensor CSV: {filename}')
...
def detect_csv_format(filename: str) -> str:
    """
    Detect which CSV format the file uses by examining the header.

    Returns:
        'generic-redfish' or 'bf3-sensor' or 'unknown'
    """
    with open(filename, 'r') as file:
        header = file.readline().strip()

        if 'power_consumed_watts' in header:
            return 'generic-redfish'
        elif 'power_envelope' in header:
            return 'bf3-sensor'
        else:
            return 'unknown'
...
def process_all_power_files():
    ...
    if not power_files:
        print("WARNING: No power files found (power-*.csv)")
        return

    print(f"Found {len(power_files)} power file(s)")
...
def main():
    ...
    # Find all power files (uncompressed CSV)
    power_files = glob.glob('power-*.csv')
